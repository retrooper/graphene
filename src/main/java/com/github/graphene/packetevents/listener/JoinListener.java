...
public class JoinListener implements PacketListener {
    private static NBTCompound DIMENSION =  new NBTCompound();
    private static NBTCompound DIMENSION_CODEC = new NBTCompound();

    public JoinListener() {
        byte[] dimensionBytes = new byte[0];
        try (InputStream dimensionInfo = JoinListener.class.getClassLoader().getResourceAsStream("RawDimensions.bytes")) {
        ...
        try (InputStream dimensionCodecInfo = JoinListener.class.getClassLoader().getResourceAsStream("RawCodec.bytes")) {
    ...
    public void onPacketSend(PacketSendEvent event) {
        ...
        if (event.getPacketType() == PacketType.Login.Server.LOGIN_SUCCESS) {
            ...
            //TODO Make this a proper post task
